...
#************************
$xajax->register(XAJAX_FUNCTION,'view_indicatorTwentyEight');
$xajax->register(XAJAX_FUNCTION,'new_indicatorTwentyEight');
$xajax->register(XAJAX_FUNCTION,'save_indicatorTwentyEight');
$xajax->register(XAJAX_FUNCTION,'edit_indicatorTwentyEight');
$xajax->register(XAJAX_FUNCTION,'update_indicatorTwentyEight');
$xajax->register(XAJAX_FUNCTION,'delete_indicatorTwentyEight');
...
function view_indicatorTwentyEight($cur_page=1,$records_per_page=20){
...
$data.="<form  name='indicatortwentyeight' id='indicatortwentyeight' method='post' action='".$PHP_SELF."'>";
$data.="<table width='100%' border='0' cellspacing='1' cellpadding='1'>";

        $data.="<tr class='evenrow'>";
            $data.="<td colspan=6 align='right'>
                        <input name='new_indicatorTwentyEight' type='button' class='formButton2' value='New Entry' onclick=\"xajax_new_indicatorTwentyEight('','','','','','','');\">
                  </td>";
        $data.="</tr>";

    $data.="<tr class='evenrow3'><td colspan='22'>";
            $data.="<table border='0' width='100%' cellspacing='1' cellpadding='1'>
                <tr class='evenrow'><th colspan='22'><div align='center'>FtF INDICATOR NUMBER 28  DETAILS</div></th></tr>";
                  
				  $data.="<tr>
								<td class='offwhite' COLSPAN='11' align='left'>
								<input type='button' class='formButton2'title='Edit' onclick=\"xajax_edit_indicatorTwentyEight(xajax.getFormValues('indicatortwentyeight'));return false;\" value='edit' /> ";
								$data.=" <input type='button' value='Delete' class='red' onclick=\"ConfirmDelete(xajax.getFormValues('indicatortwentyeight'),'delete_indicatorTwentyEight');return false;\" align='left'></td>";
								$data.="</td>";
					$data.="</tr>";	
                  ...
                  $query_string="select p.*,u.* from `tbl_indicatortwentyeight` as p join tbl_user as `u` on (`u`.`user_id` = `p`.`enteredby`)  order by p.financialYear asc";
                  
                  
                                 
                           $count=1;
                           $query_=mysql_query($query_string)or die(http(97));
                            /**************
                            *paging parameters
                            *
                            ****/
                            ...
                            $x=1;
                            while($rowIndicator=mysql_fetch_array($new_query)){
                                $color=$count%2==1?"evenrow3":"evenrow";
                            $data.="<tr class='alternating_rows'>";
                            $data.="<td>".$inc.".";
                            $data.="<input type='checkbox'  name='pk_id[]' id='pk_id".$count."' value='".$rowIndicator['pk_id']."' />";
                            $data.="<input name='loopkey[]' id='loopkey".$count."' type='hidden' value='1'/>";
                            $data.="</td>";
							$data.="<td>".strtoupper(retrieve_info_withSpecialCharactersNowordLimit($rowIndicator['name']))."</td>";
							//Pick the current username
							$stUser="select * from `tbl_user` where `user_id`='".$rowIndicator['updatedby']."'";
							...
							$data.="<td align='right'>".strtoupper(retrieve_info_withSpecialCharactersNowordLimit($rowIndicator['actualValue']))."</td>";
                            $data.="<td align='right'>".strtoupper(retrieve_info_withSpecialCharactersNowordLimit($rowIndicator['financialYear']))."</td>";
							$data.="<td><input type='button' class='formButton2' TITLE='Edit' onclick=\"xajax_edit_indicatorTwentyEight(xajax.getFormValues('indicatortwentyeight'));return false;\" value='edit' /></td>";
                            $data.="</tr>";
							$inc++;
                                     }
						...
						$data.="<tr>
								<td class='offwhite' COLSPAN='11' align='left'><input type='button' class='formButton2'title='Edit'   onclick=\"xajax_edit_indicatorTwentyEight(xajax.getFormValues('indicatortwentyeight'));return false;\" value='edit' /> ";
								$data.=" <input type='button' value='Delete' class='red' onclick=\"ConfirmDelete(xajax.getFormValues('indicatortwentyeight'),'delete_indicatorTwentyEight');return false;\" align='left'></td>";
								$data.="</td>";
						$data.="</tr>";					
...
$links=1;
	...
	if ($cur_page==1)$data.="<a href='#' onclick=\"xajax_view_indicatorTwentyEight('1','".$records_per_page."')\"><font color=red><b>1</b></font>\n</a>...".$append_bar;
	else $data.="<a href='#' onclick=\"xajax_view_indicatorTwentyEight('1','".$records_per_page."')\">1\n</a>...".$append_bar;
	//for($p=2;$p<$num_pages;$p++){
	$p=2;
	while($p<$num_pages){
if(($p>$startAt_links) and ($p<$endAt_links)){
		$data.=$p==$cur_page?"<font color=red><b>".$p."</b></font>".$append_bar:"<a href='#' onclick=\"xajax_view_indicatorTwentyEight('".$p."','".$records_per_page."')\">".$p."\n</a>".$append_bar;
	}
	$p++;
	}
	if($p==$num_pages){
			$data.="...<a href='#' onclick=\"xajax_view_indicatorTwentyEight('".$p."','".$records_per_page."')\">".$p."\n</a>".$append_bar;
			}
}


$data.=" Records: <select name='num_rec' id='num_rec' onchange=\"xajax_view_indicatorTwentyEight('".$cur_page."',this.value)\">";
	$i=1;
	$selected="";
...
function  new_indicatorTwentyEight($regionalCode,$districtCode,$subCounty,$village){
...
$data="<form  name='Indicator' id='Indicator' method='post' action='".$PHP_SELF."'>";
$data.="
<table width='600' border='0'>
        <tr>
            ...
            $data.="<table border='0' width='400' cellpadding='1' cellspacing='1'>
                <tr>
                  <th colspan='2' scope='col'>FtF INDICATOR NUMBER 28</th>
                  </tr>";
				   ...
				   $data.="<tr class='evenrow'>";
						...
						$data.="<td>";
							$data.="<input type='text' name='actualValue' id ='actualValue' onkeypress='return numbersonly(event, false)'  style='width:177px;'>";
						$data.="</td>"; 
				   $data.="</tr>";
                  ...
                  $data.="<tr class='evenrow'>";
						...
						$queryU=Execute($stUser) or die(http("ProportionOfFemales-240"));
						$rowU=FetchRecords($queryU);
						$entredby=$rowU['name'];
                ...
                $data.="<tr class='evenrow'>
                  <td>&nbsp;</td>";
                  $data.="<td><input value='Save' id='saveIndicator' name='saveIndicator' onclick=\"loadingIcon(xajax.getFormValues('Indicator'),'save_Indicator');return false;\" type='button'></td>
                </tr>
              </table>";
...
//Save indicatortwentyeight
function save_indicatorTwentyEight($formvalues){
$obj=new xajaxResponse();

$fyear=$formvalues['fyear'];
$actualValue=$formvalues['actualValue'];
$enteredby=$formvalues['enteredby'];



$stmnt_xtra="INSERT INTO `tbl_indicatortwentyeight`(`actualValue`, `financialYear`, `enteredby`, `updatedby`) 
VALUES ('".$actualValue."','".$fyear."','".$enteredby."','".$enteredby."')";
	/* $obj->alert($stmnt_va);                                            */
	@mysql_query($stmnt_xtra)or die(http(317));
    $obj->call("hidemyLoaderDiv"); 
    $obj->assign('bodyDisplay','innerHTML',congMsg("Value has been added successfully!"));
  ...
  <tr>
    ...
    <div id="bodyDisplay">
      <script language="JavaScript" type="text/javascript">
			...
			switch($_GET['linkvar']){
                            case "Indicator Number 28": 
                        ?>
                              xajax_view_indicatorTwentyEight(1,20);
