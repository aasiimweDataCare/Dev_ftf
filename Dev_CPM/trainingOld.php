...
$data.="<td><input type='button' class='formButton2' TITLE='Details' onclick=\"xajax_view_villageAgent('".$row['tbl_villageAgentId']."');return false;\" value='Details'/></td>";
$data.="</tr>";
$x++;
$m++;
}
                    ...
                    <td colspan='22'>";
                          ...
                          $data.="<table border='0' cellspacing='2' cellpadding='2' width='100%'>
                                ...
                                '".$addField."');return false;\"  style='width:150px;'>
                                    ...
                                    $data.="<tr class='evenrow'>
                                      <td colspan=''>Name of Village Agent:</td>
                                      <td colspan=''>";
                                ...
                                '".$addField."');return false;\" style='width:150px;'>
                                                                                        ...
                                                                                        $data.=$QueryManager->filterVillageAgent($trCode,$vaCode);
                                                                                            $data.="</select>";
                $data.="</td>
                                      <td colspan=''>Village Agents Code:</td>";
...
$data.="<tr class='evenrow3'>
                    ...
                    $data.="<tr class='evenrow'>
                              ...
                              <tr class='evenrow'>
                                ...
                                $stmnt2="SELECT `vg`.*, max(`vg`.`groupCode`) as `lastGroupCode`, `v`.*
                                from `tbl_villageagent_groups` as `vg`, `tbl_villageagents` as `v`
                                where `vg`.`tbl_villageAgentId`='".$vaCode."'
                                and `vg`.`tbl_villageAgentId` =`v`.`tbl_villageAgentId`";
                                           //$obj->alert($stmnt2);
                                      
                                        $q=mysql_query($stmnt2)or die(http(501));
                                         $y=1;
                                         //$rowCheck=@mysql_num_rows($q);
                                         $r=mysql_fetch_array($q);
                                         $lastGroupCode=$r['lastGroupCode'];
                                            ...
                                            where `v`.`tbl_villageAgentId`='".$vaCode."'";   
                                             $q=mysql_query($stmnt3) or die(http(520));
                                            $r=mysql_fetch_array($q);
                                            $vaCodes=$r['vAgentCode'];
                                             //$obj->alert($vaCodes);
...
$data.="<tr class='evenrow3'>
                    ...
                    $data.="<tr class='evenrow'>
                                ...
                                <th>Sex</th>
                                <th>Telephone</th>
                                <th>Received/Made e-Payment(Yes/No)</th>
                                <th>Action</th>
                            </tr>";
                            ...
                            $data.="<tr class='evenrow'>
                                ...
                                <td valign='top'>
                                  <select name='farmersEpay[]' id='farmersEpay1'>
                                    <option value=''>-select-</option>
...
$data.="<table width='100%' cellpadding='2' cellspacing='2'>
        <tr class='white1'>
        <td colspan='22'>
        <table width='100%' height='226' border='0' cellpadding='2' cellspacing='2'>
   <tr>
...
$data.="<div align='center' height='900' id='dvLoading'>
<span style='font-size:24px; margin-top:50px; font-weight:bold;'>Loading Content...</span></div>";
...
if($groupName=='' OR $groupName=='No Group'){
...
if(!$qst){ $obj->alert("CPM MIS could not make this form 7. \n
...
if(($tbl_tradersId<>''
   ...
   ($groupName<>''
    ...
    //$obj->alert($groupIdCheck);
    
    //$qCheckId=@Execute($groupIdCheck)or die(http(1052));
    ...
    if($groupName=='No Group'){
		...
		$stmnt_group="INSERT INTO `tbl_villageagent_groups`( tbl_villageagent_groupsId,`tbl_tradersId`,`tbl_villageAgentId`,
											...
											`groupStatus`,`numMembersFemale`,`numMembersMale`,
											`numMembersTotal`,`groupDistrict`,`groupSubcounty`,
											`groupVillage`) VALUES
                                            ('".$tbl_villageagent_groupsId."','".$tbl_tradersId."','".$tbl_villageAgentId."',
                                            '".$reportingPeriodVA."','".$groupName."','".$groupCode."',
											'".$groupStatus."','".$numMembersFemale."','".$numMembersMale."',
											'".$numMembersTotal."','".$district."','".$groupSubcounty."',
											'".$groupVillage."')";
		
	}else if($lastGroupCode<>null or $lastGroupCode<>'' and $groupName<>'No Group'){
...
$query=@Execute($stmnt_group)or die(http(1106));
...
$farmersage=00;
$thisYear=date('Y');
$birthYear=($thisYear-$farmersage);
$farmersDob= (''.$birthYear.'-01-01');

}else{
$farmersage=$farmersDob;
$thisYear=date('Y');
$birthYear=($thisYear-$farmersage);
$farmersDob= (''.$birthYear.'-01-01');    
...
$stmnt_grMembers="INSERT INTO `tbl_farmers`(`tbl_tradersId`, `tbl_villageAgentId`,`tbl_villageagent_groupsId`, 
                                            ...
                                            `farmersName`,`farmersDob`,`farmersVillage`, `farmersSex`, `farmersTel`,`farmers_e_pay`)
                                                        VALUES
                                                    ('".$tbl_tradersId."', '".$tbl_villageAgentId."','".$tbl_villageagent_groupsId."',
                                            '".$reportingPeriodVA."','".$groupName."','".$district."','".$groupSubcounty."',
                                            '".$nameIndividual."','".$farmersDob."','".$farmersVillage."','".$sexIndividual."', '".$telIndividual."','".$farmersEpay."')";

//$obj->alert($stmnt_grMembers);
//@mysql_query($stmnt_grMembers)or die("CPM Error-code 702 because ".mysql_error());
$query=@Execute($stmnt_grMembers)or die(http(949));

if(!$query){ $obj->alert("CPM MIS could not make this form 7. \n
                   Please make sure all the data to be supplied is correct.");
      return $obj;
     }
//@Execute($stmnt_grMembers) or die(mysql_error()." on line 945");
}
}
}else{
