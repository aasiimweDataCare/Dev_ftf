...
view_enterpriseTechAdoption($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_labourSavingTech($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_bds($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_mediaPrograms($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_youthApprentice($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_bankLoans($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_inputSales($_GET['reportingPeriod'],$_GET['financialYear'],$_GET['cur_page'],$_GET['records_per_page'],$_GET['format']);
...
view_phh($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_partnerships($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_form3($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_form4($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
tradersVasFarmerCount($_GET['semi_annual'],$_GET['year'],$_GET['format']);
...
view_form7($_GET['reportingPeriod'],$_GET['financialYear'],$_GET['fromDate'],$_GET['toDate'],$_GET['vaName'],$_GET['vaCode'],$_GET['cur_page'],$_GET['records_per_page'],$_GET['format']);
...
//-----------------------------------------
   ...
   $data .= "<tr><th rowspan='3' class='dataRow' >NO/SELECT</th>
			 <th rowspan=3 colspan='6' width='' class='dataRow'>Indicator</th>
		...
		<th colspan='20' class='dataRow' ><center>Activity Indicator Tracking Table for the for the Period 2013 - 2018 <em>, LOA=Life of Activity, T=Target,A=Actual, % Achieved=>((LOAA/LOAT)x100)</em></center></th>
			...
			<tr>
             	...
             	<th  colspan='2' rowspan='2'  class='dataRow'>type of Indicator</th>
    ...
    while ($rowoutput = @mysql_fetch_array($logicaloutput)) {
        ...
        if (@mysql_num_rows($query) > 0)
            while ($row = @mysql_fetch_array($query)) {
                ...
                $data .= "<td style='" . $two . "' rowspan='" . $rowspan . "' align='right' >" . checkExistance($rowWP['fy2'], 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $two . "' align='right' >" . checkExistance($ActualYr2, 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $three . "' rowspan='" . $rowspan . "' align='right' >" . checkExistance($rowWP['fy3'], 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $three . "' align='right' >" . checkExistance($ActualYr3, 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $four . "' rowspan='" . $rowspan . "' align='right' colspan='1' >" . checkExistance($rowWP['fy4'], 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $four . "' align='right' >" . checkExistance($ActualYr4, 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $five . "' rowspan='" . $rowspan . "' align='right' colspan='1' >" . checkExistance($rowWP['fy5'], 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $five . "' align='right' >" . checkExistance($ActualYr5, 0, 'ExistsInteger') . "</td>";

                $data .= "<td style='" . $six . "' rowspan='" . $rowspan . "' align='right' colspan='1' >" . checkExistance($rowWP['fy6'], 0, 'ExistsInteger') . "</td>";
                ...
                //======================================================================================
                //==============The actual indicator disaggregations for the ftf Report=====================
                //======================================================================================
...
function exportMelaReport ($zone, $district, $indicator_type, $subcomponent, $output, $year, $semiAnnual, $indicatorCode, $indicator,$format)
{
    ...
    for ($n = intval($queryHeader['opening_year']); $n < intval($queryHeader['closure_year']); $n++) {
        ...
        $data .= "<th width='100' class='" . $class . "'  align='right' colspan='3' >FY " . $n . "<img src='images/spacer2.png' width='15' height='0.1'></th>";
...
$title=strtoupper("".$dataForm." data for period ".$period." ");
				...
				switch($dataForm){
					...
					/* start form1 retrieval */
						$sql = $qmobj->form7RawDataToExcelAll($fromDate,$toDate,$reportingYear,$reportingPeriod);
						$result = @Execute($sql) or die(http(__line__));
						$data.="<table border=1 cellspacing=1 cellpadding=1><tr>";
						for($i = 0; $i < mysql_num_fields($result); $i++) {
							$field_info = mysql_fetch_field($result, $i);
...
function view_labourSavingTech($reportingPeriod,$year,$format){
...
<th colspan='8' ><center>Commodity Production and Marketing Activity  VALUE CHAIN DATA COLLECTION FORM/Labour Saving Technology</center></th>
</tr>";
...
$data.="<tr>
  ...
  <tr >
    ...
    <th>Time spent on job (Months)</th>
		...
		from `tbl_laboursavingtech` as `l`  join tbl_labourSavingTech_jobHolder as `Lj`
		on (`l`.`tbl_laboursavingtechId` = `Lj`.`labour_saving_tech_id`),
		tbl_mainvaluechain as `v`
		where `l`.valueChain= `v`.`name` ";
		
			if($reportingPeriod<>'') { $query_string.=" AND `l`.`reportingPeriod` = '".$reportingPeriod."' "; }
			if($financialYear<>'') { $query_string.=" AND `l`.`year` LIKE '%".$financialYear."%' "; }
			if($valueChain<>'') { $query_string.=" AND `v`.`tbl_chainId` = '".$valueChain."' "; }
 ...
 $x=1;
	$query_=mysql_query($query_string)or die(http(64));
	 /**************
	 *paging parameters
	 *
	 ****/
	 ...
	 $x=$offset+1;
	 $new_query=mysql_query($query_string." limit ".$offset.",".$records_per_page) or die(http(1082));
	
  
  while($row=mysql_fetch_array($new_query)){
			$data.="<tr class='alternating_rows'>";
			$data.="<td>".$x.".</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['labourSavingTech'])."</td>";
			$data.="<td>".substr(retrieve_info_withSpecialCharactersNowordLimit($row['valueChain']), 2)."</td>";  
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['reportingPeriods'])."</td>";				
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['labourSavingConcept'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['personResponsible'])."</td>";
			$data.="<td align='right'>".number_format(($row['amountInvestedOnTechInvestment']),2)."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['nameOfJobHolder_Lj'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['sexOfJobHolder_Lj'])."</td>";
			$dateOfEngagement = sanitizeForm2DirtyDate(substr(retrieve_info_withSpecialCharactersNowordLimit($row['dateOfEngagement_Lj']), 0, 10));
			$data.="<td>".$dateOfEngagement."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['timeSpentOnJob_Lj'])."</td>";
			$data.="</tr>";
			$x++;
			$n++;
			}
...
//view_mediaPrograms
function view_mediaPrograms($reportingPeriod,$year,$format){
...
tbl_mainvaluechain as `v`
where `m`.valueChain = `v`.`name` ";
	if($reportingPeriod<>'') { $query_string.=" AND `l`.`reportingPeriod` = '".$reportingPeriod."' "; }
	if($financialYear<>'') { $query_string.=" AND `l`.`year` LIKE '%".$financialYear."%' "; }
	if($valueChain<>'') { $query_string.=" AND `v`.`tbl_chainId` = '".$valueChain."' "; }
	if($mediaType<>'') { $query_string.=" AND `m`.`typeOfMedia` LIKE  '%".$mediaType."%' "; }
$query_string.=" order by `m`.`tbl_mediaprogramsId` desc";
 ...
 $x=1;
	 ...
	 ****/
	 ...
	 $offset = ($cur_page-1)*$records_per_page;
	 $x=$offset+1;
	 $new_query=mysql_query($query_string." limit ".$offset.",".$records_per_page) or die(http(5838));
...
function view_partnerships($reportingPeriod,$year,$format){
...
$stmnt="SELECT p.* FROM `tbl_public_private_partnership`p ORDER BY p.`tbl_partnershipId` DESC";
  $qbds=mysql_query($stmnt);
  $x=1;
  while($row=mysql_fetch_array($qbds)){
...
$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['nameOfJobHolder'])."</td>";
$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['sexOfJobHolder'])."</td>";
$data.="<td>".substr(retrieve_info_withSpecialCharactersNowordLimit($row['dateOfEngagement']), 0, 10)."</td>";
$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['timeSpentOnJob'])."</td>";
$data.="</tr>";
$x++;
$n++;
}
...
//view_form3
function view_form3($reportingPeriod,$year,$format){
...
$data.="
  <tr>
    ...
    <th rowspan='3'>Exporter Age<img src='' height='0.2' width='75'/></th>
...
//Filter parameters
					...
					if($reportingPeriod<>'') { $query_string.=" AND w.`reportingPeriod` = '".$reportingPeriod."' "; }
					if($financialYear<>'') { $query_string.=" AND w.`year` LIKE '%".$financialYear."%' "; }
					if($exName<>'') { $query_string.=" AND w.`tbl_exporterId` = '".$exName."' "; }
					if($exCode<>'') { $query_string.=" AND x.`exporterCode` LIKE '%".$exCode."%' "; }
					
					$query_string.=" ORDER BY w.`tbl_form3_exportersId` DESC";
//$obj->alert($query_string);
	$x=1;
	$query_=mysql_query($query_string)or die(http(5827));
	 /**************
	 *paging parameters
	 *
	 ****/
	...
	while($row=mysql_fetch_array($new_query)){
		...
		$data.="<tr class='alternating_rows'>";
			...
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterSex'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterCode'])."</td>";
			$returnedDate=retrieve_info_withSpecialCharactersNowordLimit($row['exporterDob']);
			...
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterVillage'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterTel'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterCropBeans'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterCropCoffee'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['exporterCropMaize'])."</td>";
			$data.="<td align='right'>".number_format(retrieve_info_withSpecialCharactersNowordLimit($row['exTradersSupplyChain']))."</td>";
			$data.="<td align='right'>".number_format(retrieve_info_withSpecialCharactersNowordLimit($row['exUnionSupplyChain']))."</td>";
...
//view_form4
function view_form4($reportingPeriod,$year,$format){
$n=1; 
$cur_page=$_SESSION['cur_page'];
$records_per_page=$_SESSION['records_per_page'];
$region=($_SESSION['region']<>''?$_SESSION['region']:$region);
...
//Mysql query to return Results from uploaded Excel sheet
	$query_string="SELECT `w` . * ,
					...
					if($reportingPeriod<>'') { $query_string.=" AND w.`reportingPeriod` = '".$reportingPeriod."' "; }
					if($financialYear<>'') { $query_string.=" AND w.`year` LIKE '%".$financialYear."%' "; }
					if($trName<>'') { $query_string.=" AND w.`tbl_traderId` = '".$trName."' "; }
					if($trCode<>'') { $query_string.=" AND x.`traderCode` LIKE '%".$trCode."%' "; }
	...
	while($row=mysql_fetch_array($new_query)){
		...
		//$data.="<tr class='".$color."'><td colspan='20'><div id='".$_SESSION['divVwDisaggregationTraders']."'></div><td></tr>";
...
//view_form7
function view_form7($reportingPeriod,$financialYear,$fromDate,$toDate,$vaName,$vaCode,$cur_page,$records_per_page,$format){
$space="&nbsp;";
...
and `v`.`display`='Yes' ";
	...
	if(!empty($reportingPeriod)){ $filterValue.="and left(`g`.`reportingPeriod` , 9) like '".$reportingPeriod."%'  ";}
	if(!empty($financialYear)){ $filterValue.="and right(`g`.`reportingPeriod` , 4) like '".$financialYear."%'  ";}
	if((!empty($fromDate)) && (!empty($toDate))){ $filterValue.="and `g`.`dateGroupStarted` between '".$fromDate."' and '".$toDate."'  ";}
	if(!empty($vaName)){ $filterValue.="and v.`tbl_villageAgentId` = '".$vaName."'  ";}
	if(!empty($vaCode)){ $filterValue.="and v.`vAgentCode` like '".$vaCode."%' ";}
...
$data.="<tr class='".$color."'>";
	...
	if(!empty($reportingPeriod)){ $filterValue.="and left(`f`.`reportingPeriod` , 9) like '".$reportingPeriod."%'  ";}
	if(!empty($financialYear)){ $filterValue.="and right(`f`.`reportingPeriod` , 4) like '".$financialYear."%'  ";}
	if((!empty($fromDate)) && (!empty($toDate))){ $filterValue.="and `f`.`memberDstart` between '".$fromDate."' and '".$toDate."'  ";}
...
//view_bankLoans
function view_bankLoans($reportingPeriod,$year,$format){
...
$stmnt="SELECT l.* FROM `tbl_bankloans` l ORDER BY l.`tbl_bankLoanId` DESC";
  $qbds=mysql_query($stmnt);
  $x=1;
  while($row=mysql_fetch_array($qbds)){
$color=$n%2==1?"evenrow":"white1";
...
$data.="<td>".$x.".</td>";
			...
			$data.="<td>".($recipientSex=='M'?$tick:$wrong)."</td>";//M
			$data.="<td>".($recipientSex=='F'?$tick:$wrong)."</td>";//F
			$data.="<td>".($recipientSex=='Joint'?$tick:$wrong)."</td>";//Joint
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['bankingInstitution'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['nameOfJobHolder'])."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['sexOfJobHolder'])."</td>";
			$dateOfEngagement = sanitizeForm2DirtyDate(substr(retrieve_info_withSpecialCharactersNowordLimit($row['dateOfEngagement']), 0, 10));
			$data.="<td>".$dateOfEngagement."</td>";
			$data.="<td>".retrieve_info_withSpecialCharactersNowordLimit($row['timeSpentOnJob'])."</td>";
			$data.="</tr>";
			$x++;
...
//view_inputSales
function view_inputSales($reportingPeriod,$financialYear,$cur_page,$records_per_page,$format){
	//===================data to be displayed=====================
	
	$data.="<table border='1' cellpadding=1 cellpadding=1 width='100%'>";
	$data.="<tr>
		<th rowspan='3'>#</th>
	...
	$query_string="select`s`.*, `sm`.*,case
	 when `sm`.`reportingPeriod` = 'Apr - Sep' then concat(replace((`sm`.`reportingPeriod`), ' -',concat((((`sm`.`year`))),'-')), ' ', `sm`.`year`)
		when `sm`.`reportingPeriod` = 'Oct - Mar' then concat(replace((`sm`.`reportingPeriod`), ' -',concat((((`sm`.`year`)-1)),'-')), ' ', `sm`.`year`)
	 else 'null'
	...
	where `s`.`id`=`sm`.`input_sales_id` ";
		...
		$filterValue = '';
		//filter parameters
		if(!empty($reportingPeriod)){ $filterValue.="and `sm`.`reportingPeriod` LIKE '".$reportingPeriod."%' ";}
		if(!empty($financialYear)){ $filterValue.="and `sm`.`year` = '".$financialYear."' ";}
	 ...
	 $x=1;
	  ...
	  while($row=mysql_fetch_array($new_query)){
		  $color=$n%2==1?"white1":"evenrow";
				...
				$data.="<td rowspan='3'>".retrieve_info_withSpecialCharactersNowordLimit($row['ReportingPeriod'])."</td>";
				$data.="<td rowspan='3'>".substr(retrieve_info_withSpecialCharactersNowordLimit($row['dateOfStartOfinputsSalesBusiness']), 0, 10)."</td>";
...
//view_bankLoans
function view_phh($reportingPeriod,$year,$format){
...
<tr class='evenrow3'>
  ...
  $stmnt="select`p`.*, `pm`.* 
from `tbl_phh` as `p`,`tbl_phh_meta_data` as `pm`  
...
function view_youthApprentice($reportingPeriod,$year,$format){
...
$stmnt="SELECT `t`.`traderName`, `y`.*, DATEDIFF( `y`.`toDate` , `y`.`fromDate` ) AS `duration` 
FROM `tbl_youthapprentice` as `y`, `tbl_traders` as `t` 
where `t`.`tbl_tradersId`=`y`.`nameofBusiness` 
ORDER BY `y`.`tbl_youthapprenticeId` DESC ";
  $qMedia=mysql_query($stmnt);
  $x=1;
  while($row=mysql_fetch_array($qMedia)){
$color=$n%2==1?"evenrow":"white1";

$data.="<tr class='".$color."'>
<td>".$x.".<input type='checkbox'  name='tbl_youthapprenticeId[]' id='tbl_youthapprenticeId".$x."' value='".$row['tbl_youthapprenticeId']."' />
...
$data.="<td>".substr(retrieve_info_withSpecialCharactersNowordLimit($row['valueChain']), 0)."</td>";
...
$data.="<td align='right'>".retrieve_info_withSpecialCharactersNowordLimit($row['duration'])." days</td>";
...
function view_bds($reportingPeriod,$year,$format){
...
order by `v`.`tbl_businessdevId` desc";
 ...
 $x=1;
  ...
  while($row=mysql_fetch_array($new_query)){
			...
			$dot=".";
			$valueChain=(substr($row['valueChain'], 1, 1)) == $dot?substr(($row['valueChain']), 2):$row['valueChain'];
			$data.="<td>".$valueChain."</td>";
...
function view_enterpriseTechAdoption($reportingPeriod,$year,$format){
...
WHERE x.`tbl_tradersId`=t.`businessTraderName`";
					if($region<>'') { $query_string.=" AND x.`traderRegion` LIKE '%".$region."%' "; }
					if($reportingPeriod<>'') { $query_string.=" AND t.`reportingPeriod` = '".$reportingPeriod."' "; }
					if($financialYear<>'') { $query_string.=" AND t.`year` LIKE '%".$financialYear."%' "; }
					if($trName<>'') { $query_string.=" AND x.`tbl_tradersId` = '".$trName."' "; }
					if($trCode<>'') { $query_string.=" AND x.`traderCode` LIKE '%".$trCode."%' "; }
...
//$obj->alert($query_string);
  ...
  $x=1;
	$query_=mysql_query($query_string)or die(http(426));
	 /**************
	 *paging parameters
	 *
	 ****/
...
function tradersVasFarmerCount($reportingPeriod,$year,$format){
...
$data.="<table width='800' id='report' BORDER='1' CELLSPACING='1'>";
 ...
 $data.="<tr class='evenrow'>
                  <th colspan='4'><div align='center'>Traders with their VAs Farmer Counts</div></th>
                  </tr>";
                  ...
                  $data.="<tr>";
					...
					$data.="<th>VA NAME</th>";
					$data.="<th>VA CODE</th>";
					$data.="<th>FARMER COUNT</th>";
				  $data.="</tr>";
                  
                  $query_string="SELECT t. * , z.`zoneName` , d.`districtName` , s.`subcountyName`
                  ...
                  if($trName<>'' and $trCode<>''){
                    ...
                    $query_string.="AND t.`traderCode` LIKE '%".$trCode."%' ";
                     ...
                     $query_string.="AND d.`districtCode` = s.`districtCode`
                           ...
                           $count=1; 
                            $query_=mysql_query($query_string)or die(http(101));
                            /**************
                            *paging parameters
                            *
                            ****/
							...
							$data.="<tr class='evenrow'>";
								$data.="<td colspan='4'><strong>".$rowTrader['traderName']."</strong></td>";
						    $data.="</tr>";
							
							/* find the vAs */
							$sql="select v.`tbl_villageAgentId`, v.`vAgentName`, v.`vAgentCode`
								from `tbl_villageagents` as `v`
								where v.`tbl_tradersId`='".$rowTrader['tbl_tradersId']."'
								order by v.`vAgentName` asc";
							  $query=Execute($sql)or die(http("TR VA's count 147"));
							  $n=1;
								while($row=FetchRecords($query)){
									...
									$sqlF="select count(f.`tbl_farmersId`) as `numFarmers`
											...
											$queryF=Execute($sqlF)or die(http("tradersVasFarmersCount 159"));
...
function setup_exporter($reportingPeriod,$year,$format){
